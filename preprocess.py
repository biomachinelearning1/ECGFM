...
def segment_mat(row, seconds, expected_sample_rate):
    ...
    for segment_i in range(20):
        if segment_i < n_segments:
            ...
            if  last_segment_i == 0:
                last_segment_i = segment_i - 1
            
            if last_old_piece == "":
                old_piece = "mimic_iv_ecg_40689238_" + str(last_segment_i) + ".mat"
            else:
                old_piece = last_new_piece

            
            new_piece = "mimic_iv_ecg_40689238_" + str(segment_i     ) + ".mat"
            
            last_save_file = last_save_file.replace( old_piece, new_piece )


            last_old_piece = old_piece 
            last_new_piece = new_piece
            
            file = Path( last_save_file) 
...
def pipeline(
    ...
    # print(meta["save_file"])
    meta["save_path"] = org_dir + meta["save_file"]
    # print(meta["save_path"])
    if "org" not in args.skip:
        ...
        # print("Va a printear los siguientes leads:")
        # print(args.leads)
        ...
        # print(" ")
        # print("Meta data after extraction and postprocessing:")
        # print(meta)
        # print(" ")
    # Preprocess data
    ...
    if "preprocessed" not in args.skip:
        ...
        # print("Va a printear los siguientes argumentos:")
    ...
    if "segmented" not in args.skip:
        ...
        # Guardar segmented_split.csv
        segmented_split = segmented[['save_file', 'path', 'sample_size']].copy()
        segmented_split.insert(0, 'idx', range(1, len(segmented_split) + 1))
        segmented_split.to_csv(os.path.join(args.processed_root, f'segmented_split.csv'), index=False)


        # Guardar test.csv
        data = []
        for index, row in segmented.iterrows():

            # parse row "path" (proviene de segmented.csv)
            path        = row['path']
            reversed_path       = path[::-1]
            index_from_end_path = reversed_path.index('/')
            path_       = reversed_path[:index_from_end_path][::-1]
            print(path_)

            # parse row "sample_size" (proviene de segmented.csv)
            sample_size = row['sample_size']

            # join row elements
            renglones   = []
            renglones.append(path_)
            renglones.append( sample_size ) 
            data.append(renglones)


        # "test.tsv" path
        save_place      = args.processed_root
        reversed_string = save_place[::-1]
        index_from_end  = reversed_string.index('/')
        index_from_end  = reversed_string.index('/', index_from_end + 1 )
        test_path       = reversed_string[index_from_end:][::-1] + 'manifests/code_15_subset10/test.tsv'
        print(test_path)
        ...
        with open(test_path, "w", newline="") as f:
            ...
            # write upper "path" (proviene de --processed_root)
            first_path = args.processed_root + 'segmented/'
            writer.writerow( ["", first_path] )
