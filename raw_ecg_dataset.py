...
class RawECGDataset(BaseDataset):
    ...
    def postprocess(self, feats, curr_sample_rate=None, leads_to_load=None):
        ...
        if self.normalize:
            ...
            for l in leads_to_load:
                ...
                if self.std[l] == 0:
                    #print("postprocess std=0.    feats[l]= "+ str(feats[l]))
                    feats[l] = (feats[l] - self.mean[l]) / 1
                else:
    ...
    def load_specific_leads(self, feats, leads_to_load, pad=True):
        print("load_specific_leads")
        print("load_specific_leads")
        print("load_specific_leads")
        print("load_specific_leads")
        print("load_specific_leads")
        print("load_specific_leads")
        print(self.leads_bucket)
        if self.leads_bucket:
            
            leads_bucket_ = set(self.leads_bucket)
            leads_to_load_ = set(leads_to_load)
            if not leads_bucket_.issubset(leads_to_load_):
                raise ValueError(
                    "Please make sure that --leads_bucket is a subset of --leads_to_load."
                )
            
            leads_to_load = list(leads_to_load_ - leads_bucket_)
            leads_to_load.sort()
    ...
    def collator(self, samples):
        ...
        for i, (source, size) in enumerate(zip(sources, sizes)):
            ...
            if diff == 0:
                ...
                padding_mask[i, :, diff:] = True
                print("collator(raw.py) True", end=' ')
            else:
                collated_sources[i], start, end = self.crop_to_max_size(source, target_size, rand=True)
                print("collator(raw.py) no voy a confiar en esa dicotomia", end=' ')
