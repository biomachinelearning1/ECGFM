...
def main(cfg: DictConfig, override_args=None):
    ...
    for subset in cfg.dataset.valid_subset.split(","):
        ...
        _dummy_batch['net_input']['padding_mask'] = torch.isnan(_dummy_batch['net_input']['source'])
        _dummy_batch['net_input']['source'] = torch.nan_to_num(_dummy_batch['net_input']['source'], nan=0.0)
        print( "_dummy_batch " + str(_dummy_batch))
        print(" ")
        ...
        with torch.no_grad():
            ...
            # for k, v in dummy["net_input"].items():
            #     print(f"{k}: {v}")
            model.eval()
            print("net_output")
        ...
        output_store = initialize_store(
            ...
            save_path=f'outputs_{subset}.npy' #cfg.common_eval.results_path,
        )
        ...
        for i, sample in enumerate(progress):
            with torch.no_grad():
                ...
                sample['net_input']['padding_mask'] = torch.isnan(sample['net_input']['source']) ##Added by suggestion from ChatGPT
                sample['net_input']['padding_mask'] = sample['net_input']['padding_mask'].all(dim=1) ##Added by suggestion from ChatGPT
                sample['net_input']['source']       = torch.nan_to_num(sample['net_input']['source'], nan=0.0) ##Added by suggestion from ChatGPT
                ...
                if not is_dummy_batch:
                    print("####### Net_Output:")
                    print(net_output)
                    print("is_dummy_batch  inside inference.py")
                    ...
                    print("ELSE is_dummy_batch  inside inference.py")
                    pass
                is_dummy_batch = False
        print("Close the Store:")
        print(data_parallel_rank)
